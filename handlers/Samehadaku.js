    ...
    getBySearch(query, message) {
        return new Promise(async (fullfill, reject) => {
            try {
                ...
                const response = await axios.get(`https://otakudesu-rest-api.fakhrijuanda12.repl.co/api/search/${query}`);
    ...
    getDetail(query, message) {
        return new Promise(async (fullfill, reject) => {
            try {
                ...
                const response = await axios.get(`https://otakudesu-rest-api.fakhrijuanda12.repl.co/api/anime/${query}`);
                const data = response.data;
                ...
                for (let eachTitle of data.list_download) {
                    ...
                    const dlEmbed = new Discord.MessageEmbed()
                        .setColor(this.client.warna.kato)
                        .setAuthor(eachTitle[0], undefined, 'https://otakudesu.org/' + query);
